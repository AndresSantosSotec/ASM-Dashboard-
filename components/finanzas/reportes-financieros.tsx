...
export const ReportesFinancieros = () => {
  ...
  const renderPaginationControls = (key: PaginationKey, totalItems: number) => {
    ...
    const pageSizeValue = isAll ? (totalItems === 0 ? 1 : totalItems) : pageSize
    const totalPages = isAll
      ? 1
      : Math.max(1, Math.ceil(totalItems / (pageSizeValue as number)))
    const safePage = Math.min(page, totalPages)
    const start = totalItems === 0 ? 0 : (safePage - 1) * (pageSizeValue as number) + 1
    const end = isAll ? totalItems : Math.min(totalItems, safePage * (pageSizeValue as number))
