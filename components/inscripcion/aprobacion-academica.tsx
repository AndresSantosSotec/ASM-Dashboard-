// components/inscripcion/aprobacion-academica.tsx
"use client"
...
import { Eye, CheckCircle, XCircle, Calendar, Loader2, ArrowLeft } from "lucide-react"
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
import AprobacionAcademicaModal from "./modal/AprobacionAcademicaModal"
import { API_BASE_URL } from "@/utils/apiConfig"

const API_URL = process.env.NEXT_PUBLIC_API_URL || API_BASE_URL

export function AprobacionAcademica() {
    ...
    async function fetchFichas() {
      try {
        const res = await fetch(
          `${API_URL}/prospectos/fichas/pendientes-academica`,
        ...
        console.error("Error cargando fichas académicas:", err)
        Swal.fire("Error", "No se pudieron cargar las fichas pendientes de aprobación académica", "error")
      }
    }
    fetchFichas()
  }, [])
  ...
  const handleAprobar = async (id: number) => {
    const result = await Swal.fire({
      title: "¿Aprobar académicamente?",
      text: "Esta acción cambiará el estado a 'Inscrito' y activará las cuotas del estudiante",
      icon: "question",
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/prospectos/${id}/aprobar-academico`, {
        method: "POST",
        headers: {
      ...
      await Swal.fire({
        icon: "success",
        title: "Aprobación académica completada",
        text: "El estudiante ha sido inscrito exitosamente",
        timer: 2000,
        showConfirmButton: false,
      })
    } catch (err) {
      console.error("Error al aprobar académicamente", err)
      Swal.fire("Error", "No se pudo completar la aprobación académica", "error")
    } finally {
  ...
  const handleSolicitarCorreccion = async (id: number) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/prospectos/${id}/solicitar-correccion`, {
        ...
        body: JSON.stringify({ 
          comentario,
          estado_destino: "Pendiente de Aprobación Financiera"
        }),
      })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle>Aprobación Académica</CardTitle>
            <Badge className="bg-green-100 text-green-800">
              <Calendar className="mr-1 h-3 w-3" /> {filteredFichas.length} Pendientes
            </Badge>
          </div>
          <CardDescription>
            Revisa y aprueba la información académica para completar la inscripción
          </CardDescription>
        </CardHeader>
                    ...
                    <TooltipProvider>
                      <div className="flex gap-2">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>Aprobar e inscribir</TooltipContent>
                        </Tooltip>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuItem
                              onClick={() => handleRetroceder(f.id, "Pendiente de Aprobación Financiera")}
                            >
